...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Maze" do
    it "returns all users that have placed an order" do
      ...
      # Changeable Start
      active_users = User.joins(orders: :user).distinct
      # Changeable End
  ...
  context "Breakthrough and Conquer" do
    it "returns all orders with Footballs and Wrestling Rings" do
      ...
      # Changeable Start
      orders = Order.joins(:items).where.not("order_items.item_id = ?", sweat)
      # Changeable End
  ...
  context "The Eliminator" do
    it "returns all orders placed 2 weeks ago" do
      ...
      # Changeable Start
      orders = Order.where("created_at >= ?", two_weeks_ago).where("created_at <= ?", last_week)
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      # Changeable Start
      most_popular_items = Item.joins(:orders).select("items.id, count(items.id) AS item_count").group("items.id").last(2)
      # Changeable Stop
